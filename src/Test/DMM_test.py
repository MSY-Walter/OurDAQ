...
class DashDMM:
    ...
    def _measurement_loop(self):
        """Hauptschleife für kontinuierliche Messungen mit EMA-RMS"""
        while self.running:
            try:
                ...
                # --- EMA-RMS Berechnung ---
                self.rms_value = math.sqrt((1 - self.ALPHA) * (self.rms_value ** 2) + self.ALPHA * (wert ** 2))

                # Display Cache aktualisieren
                with self.lock:
                ...
                if self.recording and not self.paused:
                    with self.lock:
                        ...
                        self.wert_daten.append(self.rms_value)
...
# Layout (dein Layout unverändert)
app.layout = html.Div([
    # Header
...
# Callback-Funktionen bleiben gleich, nur `update_display` und `calculate_plot_value` angepasst:
...
def update_display(n_intervals):
    ...
    # RMS-Wert direkt anzeigen
    display_text = f"{wert:.6f} {unit}"
    
    return display_text

def calculate_plot_value(wert, modus, waveform):
    # Für das Chart RMS direkt verwenden
    return wert

# Die übrigen Callbacks (Recording, Chart, CSV Download) bleiben unverändert
