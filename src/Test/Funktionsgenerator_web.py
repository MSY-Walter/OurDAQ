...
def set_frequency(freq):
    ...
    try:
        ...
        # Kontrollregister mit B28-Bit schreiben
        ...
        # Kontrollregister mit der aktuellen Wellenform (ohne B28) wiederherstellen
        if not write_to_AD9833(current_control):
            return False
        
        aktuelle_frequenz = freq
        logger.info(f"Frequenz erfolgreich gesetzt: {freq} Hz (Wellenform: {current_control:04X})")
        return True
    except Exception as e:
...
def handle_apply_configuration(n_clicks, wellenform, frequenz):
    ...
    try:
        # Zuerst Wellenform setzen
        if not set_waveform(wellenform):
            return ('❌ Fehler beim Setzen der Wellenform', 
                    f"{base_style} border-red-400 text-red-600", 
                    wave_preview)
        
        # Dann Frequenz setzen
        if not set_frequency(frequenz):
            return ('❌ Fehler beim Setzen der Frequenz', 
                    f"{base_style} border-red-400 text-red-600", 
                    wave_preview)
        
        # Wellenform explizit nach Frequenzänderung erneut bestätigen
        if not set_waveform(wellenform):
            return ('❌ Fehler beim Bestätigen der Wellenform nach Frequenzänderung', 
                    f"{base_style} border-red-400 text-red-600", 
                    wave_preview)
