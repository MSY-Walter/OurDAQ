...
# HTML/CSS/JavaScript als String
index_html = """
<!DOCTYPE html>
<html lang="de">
<head>
...
def set_voltage():
    try:
        ...
        return jsonify({'status': 'error', 'message': "Bitte gültige Spannung eingeben!"})
...
def _run_calibration():
    ...
    kalibrier_tabelle = []
    monitoring_output.append(f"[{datetime.now().strftime('%H:%M:%S')}] Starte Kalibrierung...")
    ...
    for dac_wert in range(0, 4096, sp_step):
        ...
        if current_mode == 'positive' and spannung >= 0:
        ...
        elif current_mode == 'negative' and spannung <= 0:
            kalibrier_tabelle.append([spannung, dac_wert])
            monitoring_output.append(f"[{datetime.now().strftime('%H:%M:%S')}] DAC {dac_wert:4} -> {spannung:.5f} V")
        else:
            monitoring_output.append(f"[{datetime.now().strftime('%H:%M:%S')}] DAC {dac_wert:4} -> {spannung:.5f} V (ignoriert)")
    ...
    monitoring_output.append(f"[{datetime.now().strftime('%H:%M:%S')}] Kalibrierung abgeschlossen. {len(kalibrier_tabelle)} Punkte gespeichert.")
...
def get_monitoring_data():
    return jsonify({
        'output': monitoring_output[-10:],  # Letzte 10 Zeilen für Performance
